...
<template>
  <input
    ...
    :style="label === 'name' 
      ? { minWidth: ((displayValue?.toString().length || 2) * 1.2) + 'ch', width: 'auto' }
      : { minWidth: labelLength + 'ch', width: '100%' }"
    :class="cn(
      'file:text-foreground placeholder:text-muted-foreground selection:bg-primary selection:text-primary-foreground flex h-9 min-w-0 border-l dark:border-[#ffffff20] border-[#00000050] bg-transparent px-3 py-1 shadow-xs transition-[color,box-shadow] outline-none file:inline-flex file:h-7 file:border-0 file:bg-transparent file:text-sm file:font-medium disabled:pointer-events-none disabled:cursor-not-allowed disabled:opacity-50',
      'focus-visible:border-ring focus-visible:ring-ring/50 focus-visible:ring-[3px]',
      'aria-invalid:ring-destructive/20 dark:aria-invalid:ring-destructive/40 aria-invalid:border-destructive',
      '[&::-webkit-inner-spin-button]:appearance-none [&::-webkit-outer-spin-button]:m-0 [appearance:textfield] bg-transparent focus:outline-none focus:ring-0 focus:border-none py-2',
      label === 'name' ? 'w-auto' : 'w-full',
      props.class,
      label,
